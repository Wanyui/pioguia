  ...
  results.innerHTML = items
    .map((item) => {
      ...
      const logoColumn = hasLogo
        ? `<div class="card-media grid content-between justify-items-start gap-2">${logo}<div class="card-actions flex w-full items-center justify-start gap-2">${mapLink}</div></div>`
        : "";
      return `
